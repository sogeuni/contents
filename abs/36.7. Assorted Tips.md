...
- Setting a variable to the contents of an embedded _sed_ or _awk_ script increases the readability of the surrounding [[wrapper#^SHWRAPPER|shell wrapper]]. See [[contributed-scripts#^MAILFORMAT|Example A-1]] and [[internal#^COLTOTALER3|Example 15-20]].

- Put together files containing your favorite and most useful definitions and functions. As necessary, "include" one or more of these "library files" in scripts with either the [[special-chars#^DOTREF|dot]] (**.**) or [[internal#^SOURCEREF|source]] command.
...
- A particularly clever use of [[testconstructs#^TESTCONSTRUCTS1|if-test]] constructs is for comment blocks.
...
- Using the [[internalvariables#^XSTATVARREF|$? exit status variable]], a script may test if a parameter contains only digits, so it can be treated as an integer.
...
- Using the [[dblparens|double-parentheses construct]], it is possible to use C-style syntax for setting and incrementing/decrementing variables and in [[loops1#^FORLOOPREF1|for]] and [[loops1#^WHILELOOPREF|while]] loops. See [[loops1#^FORLOOPC|Example 11-13]] and [[loops1#^WHLOOPC|Example 11-18]].

- Setting the [[internalvariables#^PATHREF|path]] and [[system#^UMASKREF|umask]] at the beginning of a script makes it more [[36.9. Portability Issues|portable]] -- more likely to run on a "foreign" machine whose user may have bollixed up the $PATH and **umask**.
...
- An [[testconstructs#^IFGREPREF|if-grep test]] may not return expected results in an error case, when text is output to stderr, rather that stdout.
