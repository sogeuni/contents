...
Hint: Use the [[basic#^CATREF|cat]] command and the appropriate [[othertypesv#^SCRNAMEPARAM|positional parameter]].
...
**Converting [[loops1#^FORLOOPREF1|for]] loops to [[loops1#^WHILELOOPREF|while]] and [[loops1#^UNTILLOOPREF|until]] loops**

Convert the _for loops_ in [[loops1#^EX22|Example 11-1]] to _while loops_. Hint: store the data in an [[Chapter 27. Arrays#^ARRAYREF|array]] and step through the array elements.
...
Given a [[special-chars#^PROCESSIDREF|process ID]] (_PID_) as an argument, this script will check, at user-specified intervals, whether the given process is still running. You may use the [[system#^PPSSREF|ps]] and [[timedate#^SLEEPREF|sleep]] commands.
...
**[[special-chars#^ASCIIDEF|ASCII]] to Integer**
...
Implement, as a script, a "safe" delete command, sdel.sh. Filenames passed as command-line arguments to this script are not deleted, but instead [[filearchiv#^GZIPREF|gzipped]] if not already compressed (use [[filearchiv#^FILEREF|file]] to check), then moved to a ~/TRASH directory. Upon invocation, the script checks the ~/TRASH directory for files older than 48 hours and [[basic#^RMREF|permanently deletes]] them. (An better alternative might be to have a second script handle this, periodically invoked by the [[system#^CRONREF|cron daemon]].)

_Extra credit:_ Write the script so it can handle files and directories [[basic#^RMRECURS|recursively]]. This would give it the capability of "safely deleting" entire directory structures.
...
Using [[Appendix T. ASCII Table#^ASCIISH|Example T-1]] as a template, write a script that prints to a file a complete [[bashver4#^UNICODEREF|Unicode]] table.

Hint: Use the -e option to [[internal#^ECHOREF|echo]]: **echo -e '\uXXXX'**, where _XXXX_ is the Unicode numerical character designation. This requires [[bashver4#^BASH42|version 4.2]] or later of Bash.
...
Given a data file with comma-separated [[special-chars#^FIELDREF|fields]], of the form:
...
The algorithm for this, expressed as a snippet of Bash [[36.7. Assorted Tips#^PSEUDOCODEREF|pseudo-code]] is:
...
It's a simple enough recipe, and _seems_ at first glance easy enough to convert into a working Bash script. The problem, though, is that Bash has [[ops#^NOFLOATINGPOINT|no native support for floating point numbers]]. So, the script writer needs to use [[mathc#^BCREF|bc]] or possibly [[C.2. Awk#^AWKREF|awk]] to convert the numbers and do the calculations. It could get rather messy . . .
...
Strip all comments from a shell script whose name is specified on the command-line. Note that the initial [[abs/part1/sha-bang/index#^SHABANGREF|#! line]] must not be stripped out.
...
Write a script that automates the process of creating [[basic#^MANREF|man pages]].
...
Do a hex(adecimal) dump on a binary file specified as an argument to the script. The output should be in neat tabular [[special-chars#^FIELDREF|fields]], with the first field showing the address, each of the next 8 fields a 4-byte hex number, and the final field the ASCII equivalent of the previous 8 fields.
