...
5. Using the [[internalvariables#^LINENOREF|$LINENO]] variable and the [[internal#^CALLERREF|caller]] builtin.
...
|   |
|---|
|A _signal_ is a message sent to a process, either by the kernel or another process, telling it to take some specified action (usually to terminate). For example, hitting a [[special-chars#^CTLCREF|Control-C]] sends a user interrupt, an INT signal, to a running program.|
...
|   |
|---|
|[[bashver3#^BASH3REF|Version 3]] of Bash adds the following [[internalvariables#^INTERNALVARIABLES1|internal variables]] for use by the debugger.

1. $BASH_ARGC
    
    Number of command-line arguments passed to script, similar to [[internalvariables#^CLACOUNTREF|$#]].
...
8. [[internalvariables#^BASHSUBSHELLREF|$BASH_SUBSHELL]]|
