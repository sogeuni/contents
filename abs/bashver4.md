...
exit $?   # In this case, exit code = 99, since that is function return.|
    ...
    |   |   |
    |---|---|
    |![[../images/caution.gif|Caution]]|Elements of the _index_ array may include embedded [[special-chars#^WHITESPACEREF|space characters]], or even leading and/or trailing space characters. However, index array elements containing _only_ _whitespace_ are _not_ permitted.
...
- The new **coproc** builtin enables two parallel [[special-chars#^PROCESSREF|processes]] to communicate and interact. As Chet Ramey states in the [[Bibliography#^BASHFAQ|Bash FAQ]] [^2] , ver. 4.01:
...
- The [[declareref.html|declare]] builtin now accepts the -l _lowercase_ and -c _capitalize_ options.
...
- [[special-chars#^BRACEEXPREF|Brace expansion]] has more options.
...
- [[bashver4#^SUBSTREXTREF4|_Substring extraction_ on _positional parameters_]] now starts with [[othertypesv#^SCRNAMEPARAM|$0]] as the _zero-index_. (This corrects an inconsistency in the treatment of positional parameters.)
...
- The new [[internalvariables#^BASHPIDREF|$BASHPID]] internal variable.
...
- Within [[testconstructs#^DBLBRACKETS|double brackets]], the **>** and **<** string comparison operators now conform to the [[localization#^LOCALEREF|locale]]. Since the locale setting may affect the sorting order of string expressions, this has side-effects on comparison tests within _[[ ... ]]_ expressions.
...
echo|
    ...
    |   |   |
    |---|---|
    |![[../images/note.gif|Note]]|The above example uses the [[escapingsection#^STRQ|**$' ... '**]] _string-expansion_ construct.|
    
- When the _lastpipe_ shell option is set, the last command in a [[special-chars#^PIPEREF|pipe]] _doesn't run in a subshell_.
...
- [[string-manipulation#^SUBSTREXTR01|Substring extraction]] uses a negative _length_ parameter to specify an offset from the _end_ of the target string.
...
[^3]: This only works with [[special-chars#^PIPEREF|pipes]] and certain other _special_ files.
